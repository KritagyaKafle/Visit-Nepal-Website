...
export function ScrollSplitCard({
  ...
  const cardsY = useTransform(scrollYProgress, [0.8, 1], [0, -100]);
      ...
      <div className="sticky top-0 flex h-screen w-full items-center justify-center overflow-hidden [perspective:1200px]">
        ...
        >
          <p className="text-sm font-medium tracking-widest text-amber-400/80 uppercase">
            Scroll down to split image & flip cards
          </p>
        </motion.div>

        <motion.div
          style={{ scale, y: cardsY, transformStyle: "preserve-3d" }}
          className="flex h-[420px] w-full max-w-5xl px-4 relative z-10"
        >
            ...
            >
              ...
              <motion.div
                className={cn(
                  ...
                  "border border-amber-500/20 bg-gradient-to-br from-white/10 to-transparent",
              ...
              >
                ...
                <h3 className="relative z-10 mb-4 text-2xl font-serif font-semibold leading-tight">
                  {card.title}
                </h3>
                <p className="relative z-10 text-sm opacity-90 line-clamp-4 leading-relaxed">{card.description}</p>
              </motion.div>
            </motion.div>
          ))}
        </motion.div>
        ...
        <motion.div
          className="absolute bottom-[15%] left-0 right-0 text-center z-20 pointer-events-none"
          style={{
            opacity: textOpacity,
            y: textY,
          }}
        >
          <p className="text-2xl font-serif tracking-tight text-amber-200/90 italic">
            Experience Nepal's Sacred Spirit
          </p>
        </motion.div>
      </div>
    </div>
